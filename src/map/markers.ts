...
import { coordinateFromValue, yearFromValue } from "./utils";
...
export async function createCompositeMarkerImage(
	...
	const resolvedIconColor = resolveMarkerCssColor(
		"var(--bases-map-marker-icon-color)",
	);
...
export class MarkerManager {
	...
	private getActiveYearForFiltering(mapConfig: any): number | null {
		const raw = mapConfig?.year;
	...
	async updateMarkers(data: { data: BasesEntry[] }): Promise<void> {
		...
		const activeYear = this.getActiveYearForFiltering(mapConfig);
		const yearProp: BasesPropertyId | null = mapConfig.yearProperty ?? null;
		...
		for (const entry of data.data) {
			...
			// Filter by year before any drawing / icon work.
			if (activeYear != null && yearProp) {
				try {
					const v = entry.getValue(yearProp);
					const y = yearFromValue(v);
					if (y == null || y !== activeYear) {
						continue;
					}
				} catch {
					continue;
			...
			try {
				...
				console.error(
					`Error extracting coordinates for ${entry.file.name}:`,
					error,
				);
			}
	...
	private async loadCustomIcons(markers: MapMarker[]): Promise<void> {
		...
		for (const markerData of markers) {
			...
			const color =
				this.getCustomColor(markerData.entry) ||
				"var(--bases-map-marker-background)";
		...
		for (const { icon, color } of compositeImagesToLoad) {
			try {
				...
				console.warn(
					`Failed to create composite marker for icon ${icon}:`,
					error,
				);
			}
		}
